      ...
      <nav style={{ flex: 1, padding: "10px 8px", overflowY: "auto", display: "flex", flexDirection: "column", gap: 2 }}>
        {nav.map(({ id, label, icon: Icon, badge }) => {
          ...
          return (
            <button
              key={id}
              onClick={() => { setModule(id); onClose?.(); }}
              style={{
                display: "flex", alignItems: "center", gap: 10, padding: "10px 12px",
                borderRadius: 8, border: "none", cursor: "pointer", width: "100%",
                textAlign: "left", background: active ? "rgba(249,115,22,0.12)" : "transparent",
                borderLeft: active ? "2px solid #f97316" : "2px solid transparent",
                color: active ? "#fed7aa" : "#4a6080", transition: "all 0.15s", justifyContent: "space-between"
              }}
            ...
            >
              ...
              {badge && (
                <span style={{ fontSize: 11, fontWeight: 900, padding: "2px 6px", background: "rgba(239,68,68,0.2)", color: "#ef4444", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 10 }}>
                  {badge}
                </span>
              )}
            </button>
          ...
          {(["pt", "en", "es"] as Lang[]).map(l => (
            <button
              ...
              style={{
                ...
                color: lang === l ? "#f97316" : "#2a3a54", transition: "all 0.15s"
              }}
            >
              {l}
            </button>
