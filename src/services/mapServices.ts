...
export interface EarthquakeFeature {
  mag: number; place: string; time: number; lat: number; lon: number; depth: number;
}
export interface FireFeature {
  lat: number; lon: number; brightness: number; confidence: string;
}
export interface InpeFeature {
  lat: number; lon: number; municipio: string; estado: string; bioma: string;
}
export interface FloodFeature {
  lat: number; lon: number; discharge: number; level: "normal" | "attention" | "critical";
}
...
export interface WeatherData {
  temperature: number; windspeed: number; humidity: number;
  precipitation: number; weathercode: number; city: string; lat: number; lon: number;
}
export interface GdacsEvent {
  lat: number; lon: number; title: string; type: string;
  alertLevel: "green" | "orange" | "red"; country: string; date: string; url: string;
}
...
export async function fetchGdacsEvents(): Promise<GdacsEvent[]> {
  try {
    const target = encodeURIComponent("https://www.gdacs.org/xml/rss.xml");
    const res = await fetch(`https://api.allorigins.win/get?url=${target}`);
    const wrapper = await res.json();
...
export async function fetchFloodData(lat: number, lon: number): Promise<FloodFeature | null> {
  try {
    const res = await fetch(`https://flood-api.open-meteo.com/v1/flood?latitude=${lat}&longitude=${lon}&daily=river_discharge&forecast_days=1`);
    const data = await res.json();
    const discharge: number = data.daily?.river_discharge?.[0] ?? 0;
    const level: FloodFeature["level"] = discharge > 500 ? "critical" : discharge > 200 ? "attention" : "normal";
...
export async function fetchWeather(lat: number, lon: number, cityName?: string): Promise<WeatherData | null> {
  try {
    ...
    const c = data.current;
    return {
      temperature: c.temperature_2m, windspeed: c.wind_speed_10m,
      humidity: c.relative_humidity_2m, precipitation: c.precipitation,
      weathercode: c.weather_code, city: cityName || `${lat.toFixed(4)}, ${lon.toFixed(4)}`, lat, lon,
    };
  } catch { return null; }
}
